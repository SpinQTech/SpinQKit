...
def has_ibmq():
    ...
    if not _PROVIDER_CHECK.checked_ibmq:
        try:
            from spinqkit.qiskit.providers.ibmq import IBMQFactory
            from spinqkit.qiskit.providers.ibmq.accountprovider import AccountProvider
...
def has_aer():
    ...
    if not _PROVIDER_CHECK.checked_aer:
        try:
            from spinqkit.qiskit.providers.aer import AerProvider
...
def is_aer_provider(backend):
    ...
    if has_aer():
        ...
        if isinstance(backend.provider(), AerProvider):
            return True
        from spinqkit.qiskit.providers.aer.backends.aerbackend import AerBackend
...
def is_basicaer_provider(backend):
    ...
    """
    from spinqkit.qiskit.providers.basicaer import BasicAerProvider
...
def is_statevector_backend(backend):
    ...
    """
    if has_aer():
        from spinqkit.qiskit.providers.aer.backends import AerSimulator, StatevectorSimulator
